    ...
    <div className="results-layout"><aside className="run-list">{comparisons.map(run => <button key={run.comparison_id} className={run.comparison_id === selectedId ? 'active' : ''} onClick={() => selectComparison(run.comparison_id)}><span><strong>{run.name}</strong><small>{run.image_count} images · seed {run.seed}</small></span><em>{run.state}</em></button>)}</aside>
      <div>{!detail ? <div className="empty">Select a saved comparison.</div> : <>
        ...
        {detail.results.length > 0 && <><div className="sample-toolbar"><label>Sample explorer<select value={sampleId || ''} onChange={e => setSampleId(e.target.value)}>{[...new Set(detail.results.map(row => row.sample_id))].map(id => <option key={id}>{id}</option>)}</select></label></div>
          ...
          <div className="sample-grid">{sampleRows.map(row => <article key={row.model_id}><h4>{row.model_id}</h4>{row.status === 'completed' ? <><div className="images"><figure><img src={`/api/comparisons/${selectedId}/assets/${row.input_asset}`} /><figcaption>Actual model input</figcaption></figure><figure><img src={`/api/comparisons/${selectedId}/assets/${row.heatmap_overlay_asset || row.heatmap_asset}`} /><figcaption>Display-only heatmap overlay</figcaption></figure></div><dl><div><dt>Raw score</dt><dd>{row.raw_image_score.toFixed(6)}</dd></div><div><dt>Decision</dt><dd>{row.prediction || 'Unavailable — not calibrated'}</dd></div><div><dt>Preprocessing</dt><dd>{row.preprocessing_source}</dd></div><div><dt>Total</dt><dd>{row.total_ms.toFixed(1)} ms</dd></div></dl>{row.visualization?.warnings?.length > 0 && <p className="notice">Display warning: {row.visualization.warnings.join(', ')}</p>}</> : <p className="error">{row.error}</p>}</article>)}</div></>}
      </>}</div></div>
  </section>
}
...
function SingleImage({ models }) {
  ...
  return <section><div className="heading"><div><p className="eyebrow">Single image</p><h2>Run one original camera image</h2><p>The selected model’s registered preprocessing always runs live before PatchCore inference.</p></div></div>
    ...
    {result && <><div className="metric-grid single-metrics"><Metric label="Raw image score" value={result.raw_image_score.toFixed(6)}/><Metric label="Decision" value={result.prediction || 'Unavailable — not calibrated'} note={result.image_threshold == null ? 'No calibrated image threshold' : `Threshold ${result.image_threshold}`}/><Metric label="Preprocessing" value={`${result.preprocessing_ms.toFixed(1)} ms`}/><Metric label="Inference" value={`${result.inference_ms.toFixed(1)} ms`}/><Metric label="Total" value={`${result.total_ms.toFixed(1)} ms`}/><Metric label="Quality flags" value={result.quality_flags.length ? result.quality_flags.join(', ') : 'None'}/></div>
      {result.visualization?.warnings?.length > 0 && <p className="notice">Display warning: {result.visualization.warnings.join(', ')}</p>}
      <div className="single-images">{[['Original', result.assets.original], ['Actual model input', result.assets.input], ['Mask', result.assets.mask], ['Display-only heatmap', result.assets.heatmap], ['Display-only overlay', result.assets.overlay]].filter(([, file]) => file).map(([label, file]) => <figure key={label}><img src={asset(file)}/><figcaption>{label}</figcaption></figure>)}</div></>}
  </section>
}
